...
def test_cli_basic_run(runner):
    with patch("lunchmoney_transaction_enhancer.cli.TransactionEnhancer") as mock_enhancer:
...
def test_cli_cron_mode(runner):
    with patch("lunchmoney_transaction_enhancer.cli.TransactionEnhancer") as mock_enhancer:
        with patch("lunchmoney_transaction_enhancer.cli.wait_for_internet_connection") as mock_wait:
            with patch("lunchmoney_transaction_enhancer.cli.send_heartbeat") as mock_heartbeat:
                with patch("lunchmoney_transaction_enhancer.cli.HEARTBEAT_URL", "http://heartbeat.com"):
...
def test_cli_missing_token(runner):
    ...
    with patch("lunchmoney_transaction_enhancer.cli.LUNCHMONEY_API_TOKEN", None):
        with capture_logs() as cap_logs:
            result = runner.invoke(main_cmd, [])
        assert any("lunchmoney_api_token is not set" in log.get("event", "") for log in cap_logs)
