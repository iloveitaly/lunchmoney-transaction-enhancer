...
from lunchmoney_transaction_enhancer.internet import is_internet_connected, wait_for_internet_connection
...
def test_is_internet_connected_false():
    with patch("socket.socket") as mock_sock:
        ...
        mock_sock.return_value.__enter__.return_value.connect.side_effect = socket.error("no internet")
        assert is_internet_connected() is False

def test_wait_for_internet_connection_immediate():
    with patch("lunchmoney_transaction_enhancer.internet.is_internet_connected") as mock_check:
